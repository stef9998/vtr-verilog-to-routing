...
def run(
    ...
    if should_run_stage(VtrStage.VPR, start_stage, end_stage):
        ...
        if "make_ft" in vpr_args:
            ...
            # reading argument "fault_rate" and delete from list
            fault_rate = 0  # default value of fault_rate
            if "fault_rate" in vpr_args:
                fault_rate = vpr_args["fault_rate"]
                vpr_args.move_to_end("fault_rate")
                vpr_args.popitem()

            # generate filename for routing resource graph
            routing_graph = temp_dir / "routing_graph.xml"
            rr_graph_file = routing_graph.name

            # set argument "write_rr_graph" with filename
            vpr_args["write_rr_graph"] = rr_graph_file

            # run vpr flow for the first time to generate routing_graph
            vtr.vpr.run(
                architecture_copy,
                pre_vpr_netlist,
                circuit_copy.stem,
            ...
            # set argument "read_rr_graph" with filename
            ...
            # run vpr flow for second time to consider faults in routing graph
            vtr.vpr.run(
                architecture_copy,
                pre_vpr_netlist,
                circuit_copy.stem,
                ...
                temp_dir=temp_dir,
                vpr_args=vpr_args,
                log_filename="vpr_ft.out",
            )

            # delete argument "read_rr_graph" from list
            if "read_rr_graph" in vpr_args:
                vpr_args.move_to_end("read_rr_graph")
                vpr_args.popitem()
